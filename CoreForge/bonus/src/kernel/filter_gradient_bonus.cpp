...
void stu_filter_gradient(float& out, const std::vector<PixelAoS>& data,
    ...
    const std::size_t y_start = 1;
    const std::size_t y_end = H - 1;
    const std::size_t x_start = 1;
    const std::size_t x_end = W - 1;
...
void stu_filter_gradient_wrapper(void* ctx) {
    ...
    static std::unordered_map<const void *, float> cached_outputs;
    ...
    std::size_t n = args->width * args->height;
    if (args->aos_data.empty()) {
        args->aos_data.resize(n);

        const float* a = args->data.a.data();
        const float* b = args->data.b.data();
        const float* c = args->data.c.data();
        const float* d = args->data.d.data();
        const float* e = args->data.e.data();
        const float* f = args->data.f.data();
        const float* g = args->data.g.data();
        const float* h = args->data.h.data();
        const float* i = args->data.i.data();
        PixelAoS* aos = args->aos_data.data();

        #pragma omp parallel for schedule(static)
        for (std::size_t k = 0; k < n; ++k) {
            aos[k] = {a[k], b[k], c[k], d[k], e[k], f[k], g[k], h[k], i[k]};
        }
    }
